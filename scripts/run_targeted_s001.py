...
"""Targeted S=0.01 sweep for critical Lambda_c identification.

Runs ~220 cases with fine Lambda spacing at S=0.01 for Re=500, 750, 1000.
The coarse Lambda grid in phase1-n256 (9 values) may have missed narrow
laminar windows at S=0.01, leaving Figure 4 (critical Lambda_c vs Re) with
only one S=0.01 data point (Re=300).

Deduplicates against active data only (no archive):
- results/phase1-n256/
- results/reentrant-expansion/
- results/targeted-s001/  (own output, for resume)
...
Usage:
    python scripts/run_targeted_s001.py [--workers N] [--battery-pause PCT]
    python scripts/run_targeted_s001.py --dry-run
"""
...
# --- Targeted S=0.01 parameter grid ---
TARGETED_CASES = {
    (500,  0.01): np.linspace(0.05, 3.0, 60).tolist(),
    (750,  0.01): np.linspace(0.05, 4.0, 80).tolist(),
    (1000, 0.01): np.linspace(0.05, 4.0, 80).tolist(),
}

# Directories to deduplicate against (active data only, no archive)
DEDUP_DIRS = [
    "results/phase1-n256",
    "results/reentrant-expansion",
]
...
def build_targeted_params():
    """Build targeted S=0.01 parameter list as (Re, S, Lambda) tuples."""
    params = []
    for (Re, S), lambdas in sorted(TARGETED_CASES.items()):
        for lam in lambdas:
...
def main():
    parser = argparse.ArgumentParser(
        description="Targeted S=0.01 sweep for critical Lambda_c (with resume)"
    )
    parser.add_argument(
    ...
    # Setup logging
    logger = configure_logging(args.outdir, "targeted-s001")

    # Build targeted parameter grid
    all_params = build_targeted_params()
    ...
    print(f"\nTargeted S=0.01 sweep at N=256 (resumable)")
