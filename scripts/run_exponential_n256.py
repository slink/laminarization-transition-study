...
"""Re-run exponential damping sweep at N=256.

The exponential damping sweep was originally run at N=128 (results/archive/exponential/).
This script re-runs the same 270-case parameter grid at higher resolution to match
the Phase 1 N=256 re-run, using exponential Richardson damping instead of linear.

Parameters (270 cases = 6 x 5 x 9):
    Re:     [100, 200, 300, 500, 750, 1000]
    S:      [0.005, 0.01, 0.05, 0.1, 0.5]
    Lambda: [0.0, 0.01, 0.05, 0.1, 0.25, 0.5, 1.0, 2.0, 5.0]
...
Usage:
    python scripts/run_exponential_n256.py [--workers N] [--battery-pause PCT]

Examples:
    # Run all 270 cases at N=256 with exponential damping
    python scripts/run_exponential_n256.py

    # Run with 4 workers
    python scripts/run_exponential_n256.py --workers 4

    # Resume after interruption (just run the same command)
    python scripts/run_exponential_n256.py

    # Disable battery monitoring
    python scripts/run_exponential_n256.py --battery-pause 0
"""
...
# Parameter grid (same as Phase 1 / original exponential sweep)
...
def build_param_dict(Re, S, Lambda, N=256, H=5.0, gamma=2.0, Sc_t=1.0, n_cycles=20):
    """Build full parameter dict for single_run with exponential damping."""
    return {
        "Re": Re,
        "S": S,
        "Lambda": Lambda,
...
def main():
    parser = argparse.ArgumentParser(
        description="Re-run exponential damping sweep at N=256 (with resume)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--outdir", type=str,
        default="results/exponential-n256",
    ...
    # Setup logging
    logger = configure_logging(args.outdir, "exponential-n256")

    # Build full parameter grid from Cartesian product
    all_params = list(itertools.product(RE_VALS, S_VALS, LAMBDA_VALS))
    ...
    print(f"Exponential damping sweep at N=256 (resumable)")
    print(f"Total cases: {n_total}")
    print(f"Already completed: {n_completed_prior}")
    print(f"Remaining: {n_pending}")
    print(f"Grid: N=256, H=5.0, gamma=2.0, Sc_t=1.0, damping=exponential")
